...
Page({
  ...
  fetchMeterData() {
    ...
    app.request.post('https://api.wutnews.net/electric/info/detail', {
      id: meter,
      area: area
    }, {
      loading: '加载电费中'
  ...
  updateLeftData(loading = true) {
    ...
    app.request.post('https://api.wutnews.net/electric/info/update', {
      id: meter,
      area: area
    }, {
      ...
      this.setData({
        'result.time': result.time.split('.')[0].replace('T', ' ').replace('-0', '/').replace('-0', '/').replace('-', '/').replace('-', '/'),
        'predict': remain
      });
    }, result => {
