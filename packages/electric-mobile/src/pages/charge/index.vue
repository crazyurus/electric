      ...
      chargeWechat(amount) {
        ...
        this.$http.post('/pay/prepare.json', param).then(result => {
          ...
          if (result.return.mweb_url) {
            ...
            const url = result.return.mweb_url + '&redirect_url=' + encodeURIComponent('/pay/callback.json?order=' + orderId);
          ...
          if (result.return.code_url) this.qrcode('请用微信扫码完成支付', 'https://api.qrserver.com/v1/create-qr-code/?data=' + encodeURIComponent(result.return.code_url));
        });
      },
      chargeAlipay(amount) {
