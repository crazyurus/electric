...
import { getInformationDetail, logout, updateInformation, getStations } from 'electric-service';
    ...
    Vue.nextTick(() => {
      ...
      getInformationDetail({
        ...
        .then(result => {
          ...
          new CountUp(
            this.$refs.todayPrice,
            0,
            Number.parseFloat(result.today.price.replace('千瓦时', '')),
            2,
            1
          ).start();
  ...
  methods: {
    charge() {
      if (this.electric.status.includes('未开户')) {
        this.$message.alert(
          '由于上一次缴费系统主服务器故障，导致部分宿舍线上显示未开户，这部分宿舍暂时只能通过线下缴费点缴费，预计下学期全部恢复。'
        );
    ...
    update() {
      ...
      updateInformation({
        ...
        .then(result => {
          this.electric.time = result.time
            .split('.')[0]
            .replace('T', ' ')
            .replace('-0', '/')
            .replace('-0', '/')
    ...
    showStation() {
      const stationButtons = getStations().map((station, index) => ({
        text: station.name,
        onClick: () => {
    ...
    miniProgram() {
      this.$navigator.open(
        'mqqapi://microapp/open?mini_appid=1109559705&fakeUrl=https://m.q.qq.com/a/s/11c84551edb3b31057d38c468a0a0818&xcxPath=pages/index/index.html&xcxSourceType=0'
      );
    }
  },
  computed: {
