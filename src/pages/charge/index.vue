      ...
      chargeWechat(amount) {
        ...
        this.$http.post('/electric/pay/prepare', param).then(result => {
          ...
          if (result.data.data.return.mweb_url) {
            const url = result.data.data.return.mweb_url + '&redirect_url=' + encodeURIComponent('https://web.itoken.team/electric/pay/callback?order=' + result.data.data.return.prepay_id);
            if (typeof tokenNative === 'undefined') location.assign('newtab:https://web.itoken.team/electric/api/wechatPay?url=' + encodeURIComponent(url));
